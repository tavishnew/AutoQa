...
export const Route = createFileRoute("/dashboard/runs/$runId/report")({
  head: ({ params }) => ({
    meta: [
      { title: `Run ${params.runId} — Report` },
      { name: "description", content: "View the AutoQA test run report." },
    ],
  }),
  beforeLoad: async () => {
...
function RunReportPage() {
  const params = useParams({ from: "/dashboard/runs/$runId/report" });
          ...
          {run && run.reportUrl ? (
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-border bg-card overflow-hidden">
                <div className="border-b border-border bg-background/50 px-4 py-3">
                  <p className="font-mono text-sm text-muted-foreground">{run.reportUrl}</p>
                </div>
                <iframe
                  src={run.reportUrl}
            ...
            <div className="rounded-xl border border-border bg-card p-12 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                This run is still {run.status}. The report will be available once the run completes.
              </p>
            ...
            <div className="rounded-xl border border-border bg-card p-12 text-center">
              ...
              <p className="mt-4 text-sm text-muted-foreground">Loading run…</p>
            </div>
          )}
        </main>
      </div>
